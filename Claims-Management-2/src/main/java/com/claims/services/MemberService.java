...
public class MemberService {
	...
	public Member validate(String userid,String pwd) {
		Member mm=new Member();
		mm.setMemberid(userid);
		mm.setPwd(pwd);
		try {
			return temp.queryForObject("SELECT * from members WHERE memberid=:memberid and pwd=:pwd", 
				getSqlParameterByModel(mm),new MemberRowMapper());
		}
		catch(Exception ex) {
			return null;
		}
	...
	public String saveMember(Member m) {
		...
		final String sql="insert into members(memberid,fname,lname,phone,email,dob,gender,address,nominee_count,insurance_type,insurance_amount,max_claim_amount,pwd) "
				+ "values(:memberid,:fname,:lname,:phone,:email,:dob,:gender,:address,:nominee_count,:insurance_type,:insurance_amount,:max_claim_amount,:pwd)";
