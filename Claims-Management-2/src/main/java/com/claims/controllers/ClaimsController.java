...
public class ClaimsController {
	...
	@GetMapping("/newmrequest")
	public String newMemberRequest(Model model) {
		...
		return "newmclaimrequest";
	}
	
	@PostMapping("/newmrequest")
	public String saveMemberRequest(Claims c,RedirectAttributes redirAttrs) {
		System.out.println(c);
		LocalDate processdate=c.getReq_date().plusDays(45);
		c.setProcess_date(processdate);
		claimsService.saveClaims(c);
		
		redirAttrs.addFlashAttribute("msg", 
				"The Claim request for "+c.getMemberid()+" has been posted."
				+ "<br>Your claim will be processed before "+processdate.format(DateTimeFormatter.ofPattern("dd-MMM-yyyy")));
		return "redirect:/mclaims";
	}
...
//			System.out.println("Search by date");			
	...
	@GetMapping("/mclaims")
	public String memberClaimRequest(Model model,ClaimSearchDTO dto) {
		System.out.println(dto.getMemberid());
		List<Claims> list=null;
		model.addAttribute("dto", dto);
